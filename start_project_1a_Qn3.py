...
for neuron_count in neurons:
    ...
    with tf.Session() as sess:
        ...
        hidden_layer = layers.dense(x, neuron_count, activation=tf.nn.relu, kernel_initializer=init.orthogonal(
        ...
        for i in range(epochs):
            ...
            if i % 100 == 0:
                print('iter %d: train err %g, test accuracy %g, neuron %d' %
                      (i, train_loss[i], test_acc[i], neuron_count))

    plt.figure(1)
    plt.plot(range(epochs), train_loss)
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Train error')
    plt.legend(['Neurons = 5', 'Neurons = 10', 'Neurons = 15',
                'Neurons = 20', 'Neurons = 25'], loc='best')
